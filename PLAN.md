...
- Handle `stop_reason == "refusal"` before reading content (Opus 5 safety classifiers), and surface a readable error rather than an empty bubble.
...
**Phase 2 — Selection & crop: ✅ DONE (2026-08-07).** Text-selection ask flow (⌘L / toolbar, staged as an anchor the chat panel shows and consumes); overlay rect-crop mode (⌘⇧A, drag with dimmed surround, Esc/click cancels); all conversion isolated in `CropGeometry` + `CropRenderer`; PNG export at 2× with a 1600 px long-edge cap; `page.selection(for:)` text fallback captured alongside every crop for text-only providers. Measured PDFKit's actual coordinate behavior rather than trusting the docs — findings in [docs/phase2-geometry.md](docs/phase2-geometry.md); it turned up two live bug classes: `page.draw` normalizes away a non-zero box origin (so `convert` output must have it subtracted), and rotation is applied by `draw` but *not* reflected in `bounds(for:)`. Machine-verified: 45 tests (new `ClaudePDFTests` target, runs headless via `xcodebuild test`), including marker-colour pixel assertions through the real `PDFViewerController` entry points at scaleFactor 0.5/1/2/4 × rotation 0/90/180/270, on both zero and non-zero box origins. Pending a 60-second human pass: the crop drag *feel* (cursor, dim, overlay hit-testing) and the anchor chip in the chat panel — the logic is covered, the pixels-on-screen aren't. Two deliberate v1 choices: cross-page drags clamp to the page under the drag's centre, and the document does not scroll while crop mode is active (the overlay swallows the events, same as the system screenshot tool) — frame the region first, or Esc out and scroll.

**Phase 3 — Anthropic provider + chat panel:** Files API upload, Messages SSE client, prompt-cache-correct request builder, citations → jump-to-page, streaming UI, Keychain settings. *Exit: question 2 shows non-zero `cache_read_input_tokens`; a citation chip scrolls to the right page.*
