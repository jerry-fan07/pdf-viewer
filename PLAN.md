...
A minimalist, smooth macOS PDF viewer with an AI side panel: highlight text or screenshot a region of the document and ask questions about it. Supports **Claude via your Pro/Max subscription** (no API key), **Claude via the Anthropic API**, and **DeepSeek via its API**. The PDF is attached once per document and treated like a cached "project"; questions are continuous within a conversation, and starting a new one (⇧⌘N) resets the conversation while keeping that cached document context.
...
protocol ChatProvider {
    ...
    /// One question, asked inside a conversation and about a document. Streams
    /// deltas back. The two are separate arguments because they have separate
    /// lifetimes: the attachment survives everything, the conversation is thrown
    /// away whenever the reader starts a new one.
    func ask(_ question: Question, in doc: DocumentAttachment, conversation: Conversation)
...
The user-visible model: **attach the document once; ask as many conversations about it as you like.** The document is the cached prefix and outlives all of them; a conversation is just what sits after it.

- **Independent questions — superseded in Phase 8.** This section originally said every question is a fresh, independent conversation, with "a later thread mode" left as an option. That mode is now the behaviour: **questions carry the ones before them**, and **New conversation** (⇧⌘N, or the button in the panel header) drops the thread while leaving the attachment — the uploaded file, the primed session, the extracted text, the cached prefix — completely untouched. Starting a conversation therefore costs nothing, which is the whole reason the two are separate things.
- Threads are held live in `ChatEngine.conversation`, never rebuilt from the history file: a reopened document has been attached afresh and no provider still remembers yesterday's thread, so restoring one would re-bill an old conversation to imply a continuity that does not exist. A reopen starts a new conversation, and the transcript draws the break.
- A provider switch keeps the conversation. Turns are portable and get replayed to whoever answers next; a session handle is not, and is dropped at the crossing.
...
**Ask (every question — the conversation replayed over the cached document):**
...
**Ask (every question):** *fork* the session this conversation is currently at — the cached PDF read is reused **and** the session that was forked is left untouched (validated in the Phase 0 spike, see [docs/phase0-spike.md](docs/phase0-spike.md)):

- **"Strictly independent" — superseded in Phase 8.** This originally said each question is independent, which is what forking *the primed session every time* buys. Questions are now continuous, so what gets forked is the fork the previous answer left behind; the first question of a conversation forks the primed session, and **New conversation** is simply that chain being dropped, which sends the next question back to forking the primed one. Immutability of whatever was forked is unchanged and is what makes both readings work off the same mechanism. Measured against CLI 2.1.223 before this was built: a fork of a fork resumes with its parent's context, and the `session_id` on the first stream record is the new fork's, not the resumed one's.
...
- **Caching / "project" semantics:** Claude Code requests the 1-hour cache TTL on subscription automatically. Prime once per document, then `--resume <primed> --fork-session` per question: follow-ups reuse the cached document read and pay mostly for the new question (spike measured ~10× cheaper than the prime). *Superseded in Phase 8:* this line went on to say questions "stay independent of each other — matching the other providers exactly", and the second half is still true while the first is inverted — a question now resumes the previous answer's fork, and all three providers are continuous within a conversation. After ~1 hour idle, the next fork re-processes the primed history uncached — same economics caveat as §7.
...
- Right-side collapsible panel; input field pinned at bottom; each Q&A rendered as its own row of an open transcript.
  - **"No conversation affordance" — superseded in Phase 8.** This originally read "no fake 'conversation' affordance in v1", which was right while every question was independent and wrong the moment they stopped being. The affordance is now real and minimal: the composer says *Ask a follow-up…* once there is something to follow, a **New conversation** button appears in the header while there is a conversation to end, and the transcript draws a labelled break wherever one conversation gave way to the next — including the break a reopen creates.
...
**Phase 8 — Continuous conversations, with a way out: ✅ DONE (2026-08-09).** Pays off the deferral written into §5 on day one — *"a later thread mode can opt into history"* — and its consequence in §6, *"no fake 'conversation' affordance in v1"*. A question now carries the questions before it, and **New conversation** (⇧⌘N, or the header button) drops that thread while leaving the attachment alone, so the first question of a new conversation is exactly as cheap as the second question of the old one. That separation is the design: `ask(_:in:conversation:)` takes the document and the thread as two arguments because they have two lifetimes. The three providers thread differently and the type says so. The API paths replay `turns` after the cache breakpoint, so the serialized bytes up to and including the document block are identical at 0 turns and at 20 — the property the caching rests on, now asserted at every conversation length on both paths (on DeepSeek as the cache's own rule: request N's messages are a byte-prefix of request N+1's, which is why a past turn is re-rendered from its stored `Question` and never from what the viewer is showing now). Claude Code replays nothing and chains forks instead: the first question forks the primed session, each one after it forks the fork the last answer left behind, so the thread accumulates while the primed session stays pristine and its cached PDF read keeps being reused. Two facts that design hangs on were unknown from the code and were measured against CLI 2.1.223 before it was written — a fork of a fork resumes with its parent's context, and the `session_id` on the *first* stream record is the new fork's, not the resumed one's (the decoder takes the first, so the opposite would have chained every question onto the primed session and silently degraded to independence with nothing to flag it). Three smaller things the shape forced. **A handle is not portable**: crossing to another provider keeps the turns and drops the session id, and `handledTurns` records how much of the thread that session already holds so a stopped answer — which leaves no fork behind but is still on screen, and still context — gets replayed rather than lost. **An empty answer is not a turn**: replaying half of one puts two `user` messages side by side, which both APIs reject, so it is dropped whole. **A past crop is described, not re-uploaded**: ten follow-ups must not re-send ten screenshots, and it keeps the paths equivalent — DeepSeek cannot see one at all and Claude Code still has the real image in its session. History stays display-only: a reopen has a freshly attached document and no provider remembers yesterday's thread, so restoring one would re-bill an old conversation to imply a continuity that doesn't exist. The transcript says which is which — `threadID` per card, persisted, with a labelled break drawn wherever it changes and the timeline rail restarting there; files written before threads existed read back as the one conversation they were. Machine-verified: clean build; **333/333 unit tests pass**, 20 of them new — the document block unmoved at 0–4 turns, the thread replayed after it and never before it, role alternation, empty turns dropped whole on both API paths, DeepSeek's byte-prefix chain across four requests, a past crop described rather than re-sent while the live one is still sent in full, the CLI prompt retelling a conversation only when the session cannot already know it, and in the engine: a follow-up handed the turns before it, a new conversation handed none of them *and re-preparing nothing* (asserted on the attach count), cards stamped with the conversation they were asked in, the handle advancing per answer and dropped at a new conversation, a failed question not joining, and a stopped one joining but marked for replay. **A pre-existing hang, found while testing this and fixed here.** Scrolling the chat panel *while an answer was streaming* wedged the app: main thread at 100%, permanently, and still spinning long after the answer had finished — reported as "it crashes as soon as I scroll". It is not a crash and there is no crash report; it is an unterminating SwiftUI transaction loop, which a sample of the wedged process showed as `LazyStack.measureEstimates` / `_LazyLayout_Subviews.apply` / `LazyVStackLayout.initialPlacement` with no app frames but `QACard` copies inside the transcript's `ForEach`. Bisected against a clean checkout of the pre-Phase-8 tree, which wedges identically — this predates threading, which only made it easy to hit. Isolated to three cases: scrolling with no answer is fine, scrolling a finished transcript is fine, scrolling *during* a stream wedges. Two changes, and the order matters. The transcript is no longer a `LazyVStack`: a stack whose row placements are resolved on demand, against content whose height changes on every delta, while the reader drags the same scroll view, is a feedback loop that need not settle — and the transcript is capped at 200 cards, which lays out eagerly in about 25 ms measured at 40 heavy answers. And the panel no longer follows the stream on every character: `scrollTo` fired per delta, roughly a thousand times an answer, which made scrolling during an answer pointless — the view yanked back every 20 ms — for about a thousand times the work the job needs. Whether it also fed the loop is untested and not claimed: the lazy stack was replaced at the same time. It now follows when a question is asked and when an answer finishes — twice. Verified in the running app on the offline mock provider, driving real scroll-wheel events against a streaming answer: 100% CPU sustained and unrecoverable before, 0.2–0.4% settling to zero after, across three runs including a 120-event burst, with the app still answering the accessibility API. The first fix attempt — the scroll change alone — was measured and *did not* fix it, which is what sent the search to the lazy stack.

**Pending a manual pass:** a real multi-turn conversation on each of the three providers, ⇧⌘N actually firing in the running app (the header button carries the feature regardless, and a hidden-button shortcut can be shadowed by a main-menu key equivalent — an automated attempt at it was swallowed by the terminal driving the test), and the cache-hit percentages across a long thread — the per-answer "% cached" line is where a regression here would show first. Known limitation, deliberately not fixed: the replayed thread is not itself cached on the Anthropic path. The fix is a second, rolling `cache_control` on the current turn, and it is left out because the *first* breakpoint's live cache-hit behaviour is still unverified for want of an API key, and stacking an unverified optimisation on an unverified one is the wrong order.
