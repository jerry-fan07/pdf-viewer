...
enum AnthropicRequestBuilder {
    ...
    /// with the conversation's turns replayed after it, so the prompt grows as
    /// the thread does. Half the cap leaves room for both alongside the answer on
    /// the largest documents, and 64K is already two orders of magnitude past
    /// what a reading answer plus its thinking spends.
    ///
